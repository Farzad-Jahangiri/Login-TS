...
function PasswordInput(props: textInputProps) {
  ...
  const [showPass, setShowPass] = useState(See)
  ...
  return (
    ...
    >
      ...
      <span className="absolute left-2" onClick={()=>{setShowPass(prev=>prev==See?UnSee:See)}}>{showPass}</span>
      <input
        onChange={props.onChange}
        value={props.value}
        id="passwordInput"
        type={`${showPass==See?"password":"text"}`}
